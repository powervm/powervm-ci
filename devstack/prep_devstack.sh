...
usage () {
    ...
    echo "        are a comma or whitespace separated list of patches of the form 82/2535/5 or"
    ...
    echo "    -f (optional): Force flag. This forces zuul to use the master branch and no"
    echo "        additional patches will be applied to any projects before the test run."
...
if $FORCE; then
    ...
    for v in ZUUL_BRANCH ZUUL_PROJECT BASE_LOG_PATH; do
        if ! [ $"$v" ]; then
            echo '$'$v' not set.  Run with -f to assume ZUUL_BRANCH=master and skip loading a patch set.'
            exit -1
        fi
    done
fi

# Pull in any changes to powervm-ci
cd /opt/stack/powervm-ci/
git checkout master
git pull
...
# Add the ssh key and configure git globals
sudo chmod 400 /opt/nodepool-scripts/osci_rsa
sudo chown jenkins:jenkins /opt/nodepool-scripts/osci_rsa
eval `ssh-agent -s`
ssh-add /opt/nodepool-scripts/osci_rsa
git config --global user.email "openstack-ci@us.ibm.com"
git config --global user.name "openstack-ci"
ssh-keyscan -t rsa -p 29418 morpheus.pok.stglabs.ibm.com > ~/.ssh/known_hosts
...
# If running pypowervm < 1.1.2 this will apply the session config patch in addition
# to any pypowervm patches specified when calling this script. If running 1.1.2 or
# higher versions of pypowervm, this will only apply patches if they are specified
# when calling this script. When pypowervm is modified through this patching process,
# it needs to be removed from the various requirements files to ensure stacking doesn't
# overwrite patched version.
if [ ! -z "$pypowervm_patch_list" ]; then
    # Pull down pypowervm.  Apply the RMC patch that determines the IP address from
    # the RMC address.
...
# the tempest install cause stacking to fail. Removing these from u-c
