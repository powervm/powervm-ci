...
def find_target_vms(adpt):
    """Find all vms created for tempest testing."""
...
def scrub_storage(adpt, lpar_wraps):
    ftsk = tx.FeedTask(
        'scrub_stg', pvm_vios.VIOS.getter(adpt, xag=[const.XAG.VIO_SMAP]))
    tsk_stor.add_lpar_storage_scrub_tasks(
        [lwrap.id for lwrap in lpar_wraps], ftsk, lpars_exist=False)
    ftsk.execute()
...
def main():
    try:
        ...
        # Find the tempest VMs that are paired to this system.
        lpar_wraps = find_target_vms(adpt)

        # Power off all lpars
        for lpar_w in lpar_wraps:
            power.PowerOp.stop(
                lpar_w, opts=power_opts.PowerOffOpts().vsp_hard())

        # Delete the VMs. This has to happen before the storage scrub.
        delete_lpars(adpt, lpar_wraps)

        # Remove stale vSCSI mappings and backing storage.
        scrub_storage(adpt, lpar_wraps)
