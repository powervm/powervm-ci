...
if $CONTROL; then
    ssh $logserver_user@$logserver_ip "mkdir -p $logserver_path/control"
    scp $stack_log_path/*.gz $logserver_user@$logserver_ip:$logserver_path/control
    scp $stack_log_path/*powervm_os_ci* $logserver_user@$logserver_ip:$logserver_path
else
    ssh $logserver_user@$logserver_ip "mkdir -p $logserver_path/compute"
    scp $stack_log_path/*.gz $logserver_user@$logserver_ip:$logserver_path/compute
fi
